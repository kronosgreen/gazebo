...
#include "plugins/ROSSimpleTrackedVehiclePlugin.hh"
...
GZ_REGISTER_MODEL_PLUGIN(ROSSimpleTrackedVehiclePlugin)

ROSSimpleTrackedVehiclePlugin::~ROSSimpleTrackedVehiclePlugin()
{
...
void ROSSimpleTrackedVehiclePlugin::Load(physics::ModelPtr _model,
                                      sdf::ElementPtr _sdf)
{
  GZ_ASSERT(_model, "ROSSimpleTrackedVehiclePlugin: _model pointer is NULL");
  GZ_ASSERT(_sdf, "ROSSimpleTrackedVehiclePlugin: _sdf pointer is NULL");

  if (_model->GetWorld()->Physics()->GetType() != "ode")
  {
    gzerr << "Tracked vehicle simulation works only with ODE." << std::endl;
    throw std::runtime_error("ROSSimpleTrackedVehiclePlugin: Load() failed.");
  }

  ROSTrackedVehiclePlugin::Load(_model, _sdf);

  if (!_sdf->HasElement("body"))
  {
    gzerr << "ROSSimpleTrackedVehiclePlugin: <body> tag missing." << std::endl;
    throw std::runtime_error("ROSSimpleTrackedVehiclePlugin: Load() failed.");
  }

  if (!_sdf->HasElement("left_track"))
  {
    gzerr << "ROSSimpleTrackedVehiclePlugin: <left_track> tag missing."
          << std::endl;
    throw std::runtime_error("ROSSimpleTrackedVehiclePlugin: Load() failed.");
  }
  ...
  {
    gzerr << "ROSSimpleTrackedVehiclePlugin: <right_track> tag missing."
          << std::endl;
    throw std::runtime_error("ROSSimpleTrackedVehiclePlugin: Load() failed.");
  }
  ...
  if (this->body == nullptr)
  {
    gzerr << "ROSSimpleTrackedVehiclePlugin: <body> link does not exist."
          << std::endl;
    throw std::runtime_error("ROSSimpleTrackedVehiclePlugin: Load() failed.");
  }
  else
  {
    gzmsg << "ROSSimpleTrackedVehiclePlugin: Successfully added robot body link "
  ...
  {
    gzerr << "ROSSimpleTrackedVehiclePlugin: <left_track> link does not exist."
          << std::endl;
    throw std::runtime_error("ROSSimpleTrackedVehiclePlugin: Load() failed.");
  }
  else
  {
    gzmsg << "ROSSimpleTrackedVehiclePlugin: Successfully added left track link "
  ...
  {
    gzerr << "ROSSimpleTrackedVehiclePlugin: <right_track> link does not exist."
          << std::endl;
    throw std::runtime_error("ROSSimpleTrackedVehiclePlugin: Load() failed.");
  }
  else
  {
    gzmsg << "ROSSimpleTrackedVehiclePlugin: Successfully added right track link "
  ...
  if (_sdf->HasElement("left_flipper"))
  {
    ...
    while (flipper)
    {
      ...
      {
        gzerr << "ROSSimpleTrackedVehiclePlugin: <left_flipper> link '"
              << flipperName << "' does not exist." << std::endl;
      }
      else
      {
        gtracks[Tracks::LEFT].push_back(flipperLink);
        gzmsg << "ROSSimpleTrackedVehiclePlugin: Successfully added left flipper "
  ...
  if (_sdf->HasElement("right_flipper"))
  {
    ...
    while (flipper)
    {
      ...
      if (flipperLink == nullptr)
      {
        gzerr << "ROSSimpleTrackedVehiclePlugin: <right_flipper> link '"
              << flipperName << "' does not exist." << std::endl;
      }
      else
      {
        gtracks[Tracks::RIGHT].push_back(flipperLink);
        gzmsg << "ROSSimpleTrackedVehiclePlugin: Successfully added right flipper "
                 "link '" << flipperName << "'" << std::endl;
      }
...
void ROSSimpleTrackedVehiclePlugin::Init()
{
  ROSTrackedVehiclePlugin::Init();

  gzmsg << "Finished Tracked Init()" << std::endl;
  ...
  this->beforePhysicsUpdateConnection =
      event::Events::ConnectBeforePhysicsUpdate(
          std::bind(&ROSSimpleTrackedVehiclePlugin::DriveTracks, this,
                    std::placeholders::_1));
}

void ROSSimpleTrackedVehiclePlugin::Reset()
{
  ROSTrackedVehiclePlugin::Reset();
}

void ROSSimpleTrackedVehiclePlugin::SetTrackVelocityImpl(double _left,
                                                      double _right)
{
...
void ROSSimpleTrackedVehiclePlugin::SetLeftTrackVelocityImpl(const std_msgs::Float64& _left)
{
  this->trackVelocity[Tracks::LEFT] = _left.data;
}
void ROSSimpleTrackedVehiclePlugin::SetRightTrackVelocityImpl(const std_msgs::Float64& _right)
{
  this->trackVelocity[Tracks::RIGHT] = _right.data;
}

void ROSSimpleTrackedVehiclePlugin::UpdateTrackSurface()
{
  auto& gtracks = globalTracks.at(this->body);
...
void ROSSimpleTrackedVehiclePlugin::SetGeomCategories()
{
...
size_t ROSSimpleTrackedVehiclePlugin::GetNumTracks(const Tracks side) const
{
  auto& gtracks = globalTracks.at(this->body);
  return gtracks[side].size();
}

void ROSSimpleTrackedVehiclePlugin::DriveTracks(
...
ignition::math::Vector3d ROSSimpleTrackedVehiclePlugin::ComputeFrictionDirection(
...
double ROSSimpleTrackedVehiclePlugin::ComputeSurfaceMotion(const double _beltSpeed,
...
ROSSimpleTrackedVehiclePlugin::ContactIterator
ROSSimpleTrackedVehiclePlugin::ContactIterator::operator++()
{
...
ROSSimpleTrackedVehiclePlugin::ContactIterator::ContactIterator()
...
ROSSimpleTrackedVehiclePlugin::ContactIterator::ContactIterator(
    const ROSSimpleTrackedVehiclePlugin::ContactIterator &_rhs)
{
...
ROSSimpleTrackedVehiclePlugin::ContactIterator
ROSSimpleTrackedVehiclePlugin::ContactIterator::begin(
    dBodyID _body, dGeomID _geom1, dGeomID _geom2)
{
...
ROSSimpleTrackedVehiclePlugin::ContactIterator
ROSSimpleTrackedVehiclePlugin::ContactIterator::end()
{
  return ContactIterator(true);
}

bool ROSSimpleTrackedVehiclePlugin::ContactIterator::operator==(
    const ROSSimpleTrackedVehiclePlugin::ContactIterator &_rhs)
{
...
ROSSimpleTrackedVehiclePlugin::ContactIterator
ROSSimpleTrackedVehiclePlugin::ContactIterator::operator++(int /*_unused*/)
{
...
ROSSimpleTrackedVehiclePlugin::ContactIterator::reference
ROSSimpleTrackedVehiclePlugin::ContactIterator::operator*()
{
  if (!this->initialized)
    ++(*this);
...
ROSSimpleTrackedVehiclePlugin::ContactIterator::pointer
ROSSimpleTrackedVehiclePlugin::ContactIterator::operator->()
{
  if (!this->initialized)
    ++(*this);
...
ROSSimpleTrackedVehiclePlugin::ContactIterator::pointer
ROSSimpleTrackedVehiclePlugin::ContactIterator::getPointer()
{
  if (!this->initialized)
    ++(*this);
...
bool ROSSimpleTrackedVehiclePlugin::ContactIterator::operator!=(
    const ROSSimpleTrackedVehiclePlugin::ContactIterator &_rhs)
{
  return !ROSSimpleTrackedVehiclePlugin::ContactIterator::operator==(_rhs);
}
